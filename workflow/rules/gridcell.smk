...
rule extract_gridcell_date_xyz:
    input:
        rules.unpack_gridcell_region.output[0],
    output:
        temp("results/gridcell/{year}-{month}-{day}/{gridcell_field}/{region}.xyz"),
...
rule extract_gridcell_developmentclass_xyz:
    input:
        rules.unpack_gridcell_region.output[0],
    output:
...
rule rasterize_gridcell_xyz:
    input:
        raster=rules.extract_gridcell_param_xyz.output[0],
    ...
    container:
        "container.sif"
    script:
        "../scripts/rasterize_gridcell_xyz.R"
...
rule patch_gridcell_raster:
    ...
    container:
        "container.sif"
    script:
        "../scripts/patch_gridcell_raster.R"
